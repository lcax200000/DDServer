...
wb_boolean(struct write_block *wb, int boolean) {
	char n = COMBINE_TYPE(TYPE_BOOLEAN, boolean ? 1 : 0);
	wb_push(wb, &n, 1);
}
...
wb_integer(struct write_block *wb, lua_Integer v) {
	...
	if (v == 0) {
		char n = COMBINE_TYPE(type, TYPE_NUMBER_ZERO);
		wb_push(wb, &n, 1);
	}
	else if (v != (int)v) {
		char n = COMBINE_TYPE(type, TYPE_NUMBER_QWORD);
		long long int v64 = v;
		wb_push(wb, &n, 1);
		...
		int v32 = (int)v;
		char n = COMBINE_TYPE(type, TYPE_NUMBER_DWORD);
		wb_push(wb, &n, 1);
		wb_push(wb, &v32, sizeof(v32));
	}
	else if (v < 0x100) {
		char n = COMBINE_TYPE(type, TYPE_NUMBER_BYTE);
		wb_push(wb, &n, 1);
		char byte = (char)v;
		wb_push(wb, &byte, sizeof(byte));
	}
	else if (v < 0x10000) {
		char n = COMBINE_TYPE(type, TYPE_NUMBER_WORD);
		wb_push(wb, &n, 1);
		unsigned short word = (unsigned short)v;
...
wb_real(struct write_block *wb, double v) {
	char n = COMBINE_TYPE(TYPE_NUMBER, TYPE_NUMBER_REAL);
	wb_push(wb, &n, 1);
...
wb_string(struct write_block *wb, const char *str, int len) {
	if (len < MAX_COOKIE) {
		char n = COMBINE_TYPE(TYPE_SHORT_STRING, len);
		wb_push(wb, &n, 1);
		if (len > 0) {
			wb_push(wb, str, len);
		}
		...
		if (len < 0x10000) {
			...
			wb_push(wb, &n, 1);
			unsigned short x = (unsigned short)len;
			wb_push(wb, &x, 2);
		}
		else {
...
wb_table_array(lua_State *L, struct write_block * wb, int index, int depth) {
	...
	if (array_size >= MAX_COOKIE - 1) {
		char n = COMBINE_TYPE(TYPE_TABLE, MAX_COOKIE - 1);
		wb_push(wb, &n, 1);
		wb_integer(wb, array_size);
	}
	else {
		char n = COMBINE_TYPE(TYPE_TABLE, array_size);
		wb_push(wb, &n, 1);
	}
...
wb_table_metapairs(lua_State *L, struct write_block *wb, int index, int depth) {
	char n = COMBINE_TYPE(TYPE_TABLE, 0);
	wb_push(wb, &n, 1);
...
get_integer(lua_State *L, struct read_block *rb, int cookie) {
	...
	case TYPE_NUMBER_BYTE: {
							   char n;
							   char * pn = (char*)rb_read(rb, sizeof(n));
							   if (pn == NULL)
								   invalid_stream(L, rb);
							   n = *pn;
							   return n;
	...
	case TYPE_NUMBER_WORD: {
							   unsigned short n;
							   unsigned short * pn = (unsigned short*)rb_read(rb, sizeof(n));
							   if (pn == NULL)
								   invalid_stream(L, rb);
	...
	case TYPE_NUMBER_DWORD: {
								int n;
								int * pn = (int*)rb_read(rb, sizeof(n));
								if (pn == NULL)
									invalid_stream(L, rb);
								memcpy(&n, pn, sizeof(n));
								return n;
	...
	case TYPE_NUMBER_QWORD: {
								long long int n;
								long long int * pn = (long long int *)rb_read(rb, sizeof(n));
								if (pn == NULL)
									invalid_stream(L, rb);
								memcpy(&n, pn, sizeof(n));
								return n;
...
unpack_table(lua_State *L, struct read_block *rb, int array_size) {
	if (array_size == MAX_COOKIE - 1) {
		char type;
		char *t = (char *)rb_read(rb, sizeof(type));
		if (t == NULL) {
			invalid_stream(L, rb);
		}
		type = *t;
...
push_value(lua_State *L, struct read_block *rb, int type, int cookie) {
	...
	case TYPE_LONG_STRING: {
							   if (cookie == 2) {
								   unsigned short *plen = (unsigned short *)rb_read(rb, 2);
								   if (plen == NULL) {
									   invalid_stream(L, rb);
								   }
								   unsigned short n;
								   ...
								   unsigned int *plen = (unsigned int *)rb_read(rb, 4);
								   if (plen == NULL) {
									   invalid_stream(L, rb);
								   }
								   unsigned int n;
